...
```{r, include=FALSE}
base_branch = c("AD", "AG", "AR", "AV", "CM", "CY", "EN", "FA", "FM", "IN", "MI", "MP", 
                "MS", "OD", "EOD", "QM", "SC", "TC")
...
tidy_oer_senior = oers1 %>% unnest_tokens(srNarrativeWords, srNarrative) %>% 
  ...
  count(branch, srNarrativeWords, sort = TRUE)
...
Link to the "bind_log_odds" function is [here](https://github.com/juliasilge/tidylo/blob/master/R/bind_log_odds.R)
...
custom_stop_words<-c("i","in","with","an","have", "the", "is", "a", "senior", "rate",
                     "for","to","and","of", "has", "he", "his", "her","she",
                     "be", "career", "as", "by",
                     "as", "at", "this", "who", "rated", "commander", "command")
...
bigram_counts_maneuver <- bigram_filter %>% unite(bigram, word1, word2, sep = " ") %>% 
...
bigram_counts_logistics <- bigram_filter %>% unite(bigram, word1, word2, sep = " ") %>% 
...
dfjoined_bigram_logistics <- left_join(dfjoined_bigram_logistics, 
                                       freq2_bigrams_logistics, by = "srLabel")
...
unweighted_log_odds_bigrams_ordered_logistics$srLabel <-
  factor(unweighted_log_odds_bigrams_ordered_logistics$srLabel, 
         levels = c("Most Qualified", "Highly Qualified"))
```
